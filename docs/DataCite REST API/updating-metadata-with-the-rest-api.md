---
title: Updating DOIs with the REST API
excerpt: ''
deprecated: false
hidden: false
metadata:
...
To update an existing record, you will make a PUT request to `https://api.test.datacite.org/dois/{id}` —replacing `{id}` with the DOI you wish to update—with a JSON payload in the request. See the [API Reference](ref:put_dois-id) for more information.
