...
next:
  description: ''
---
Almost any meaningful use of the DataCite REST API will involve some level of record retrieval. The responses from each of your requests fall into two basic categories: Singletons (i.e., a single record) and Lists (i.e., a list of records). This section of the guide covers singletons, and the next section ([Retrieving a list of DOIs](doc:api-get-lists)) will cover lists. 

**Singletons** are single results, e.g. the metadata for a specific DOI. For example, retrieving the metadata record for the DOI `10.5438/0012` from the command line with [cURL](https://curl.haxx.se/docs/manpage.html) can be done as follows:
...
The [API reference](https://support.datacite.org/reference/introduction) provides example code for Node, Ruby, Javascript and Python.
...
[/block]
Next we'll look at [Retrieving a list of DOIs](doc:api-get-lists).