...
*  If you are not yet a DataCite Member or User, you can [obtain credentials to our DOI Registration System](doc:obtain-test-account)
...
The response will the URL registered for the DOI you requested. 
[block:callout]
{
  "type": "warning",
  "title": "Warning",
  "body": "If probably got a wrong credential error when retrieving the example DOI 10.5438/0012. Use a DOI name that belongs to your organization."
}
[/block]
### Fetch the metadata
...
The DOI suffix (the part after the prefix 10.xxxx assigned to you) can be any alphanumeric string, as long as it hasn't been used before for your prefix.  Limited punctuation is allowed, the approved character set for DOI suffixes is: "a-z", "A-Z", "0-9" and "-._;()/".  An expanded set of characters was supported prior to July 2018, so you may see DOIs with other characters as well. (such as #, +, <, and >). An example of a DOI name would be `10.5438/0012`.
...
```shell
$ curl -X GET --user username:password https://mds.test.datacite.org/doi/10.5072/JQX3-61AT
...
```shell
# PUT /metadata/10.5072
$ curl -H "Content-Type: application/xml;charset=UTF-8" -X PUT -i --user username:password -d @10.5072/JQX3-61AT.xml https://mds.test.datacite.org/metadata/10.5072
```

This will register metadata with a random DOI using the 10.5072 prefix. The DOI in the metadata file will be overwritten and can therefore be left empty.
...
$ curl -H "Content-Type: application/plain;charset=UTF-8" -X DELETE -i --user username:password  https://mds.test.datacite.org/metadata/10.5072/JQX3-61AT
```
...
```shell
# DELETE /doi/10.5072/JQX3-61AT
$ curl -H "Content-Type: application/plain;charset=UTF-8" -X DELETE -i --user username:password  https://mds.test.datacite.org/doi/10.5072/JQX3-61AT
```
