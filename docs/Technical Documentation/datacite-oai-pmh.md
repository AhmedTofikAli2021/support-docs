...
Store ([MDS](doc:metadata-store-mds-user-documentation) using the Open Archives Initiative Protocol for Metadata Harvesting ([OAI-PMH](http://www.openarchives.org/pmh/)).
[block:api-header]
{
...
[Datacite OAI schema web site](https://schema.datacite.org/oai/oai-1.0/).
...
You can use custom solr search queries in your setspec. Therefore the
solr query string must be base64url encoded, see [RFC 4648](https://tools.ietf.org/html/rfc4648#section-5), and
appended to any normal setspec or the empty string separated by a tilde
(`~`). There shall be *no* tailing padding character (`=`).

### Examples

Currently the solr query params `q` and `fq` are supported. 

- **solr query string**: [`q=laser&fq=resourceTypeGeneral:dataset`](https://oai.datacite.org/oai?verb=ListRecords&metadataPrefix=oai_datacite&set=~P3E9bGFzZXImZnE9cmVzb3VyY2VUeXBlR2VuZXJhbCUzQWRhdGFzZXQ)
- **base64url**: [`~cT1sYXNlciZmcT1yZXNvdXJjZVR5cGVHZW5lcmFsOmRhdGFzZXQK`](https://oai.datacite.org/oai?verb=ListRecords&metadataPrefix=oai_datacite&set=~cT1sYXNlciZmcT1yZXNvdXJjZVR5cGVHZW5lcmFsOmRhdGFzZXQK)
- **set name member**: [`TIB~cT1sYXNlciZmcT1yZXNvdXJjZVR5cGVHZW5lcmFsOmRhdGFzZXQK`](https://oai.datacite.org/oai?verb=ListRecords&metadataPrefix=oai_datacite&set=TIB~cT1sYXNlciZmcT1yZXNvdXJjZVR5cGVHZW5lcmFsOmRhdGFzZXQK)
- **set name data center**: [`TIB.GFZ~cT1sYXNlciZmcT1yZXNvdXJjZVR5cGVHZW5lcmFsOmRhdGFzZXQK`](https://oai.datacite.org/oai?verb=ListRecords&metadataPrefix=oai_datacite&set=TIB.GFZ~cT1sYXNlciZmcT1yZXNvdXJjZVR5cGVHZW5lcmFsOmRhdGFzZXQK)
[block:callout]
{
  "type": "info",
