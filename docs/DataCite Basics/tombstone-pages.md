...
In general, the [Best Practices for DOI Landing Pages](doc:landing-pages) still apply. The tombstone page should provide a full bibliographic citation. If the item is no longer available, then include the citation for the item that was formerly hosted on that page. If it is still available, the citation can include how to access the item at the new location. In this way, users can verify they have found the correct item (or its last resting place).

The tombstone page should contain the DOI itself in both a human-readable and machine-readable format. The only exception is that the user does not need to be able to access the item directly from the landing page, since that item has been removed.
