...
Draft DOIs may be updated to either Registered or Findable DOIs. Registered and Findable DOIs may not be returned to the Draft state, which means that changing the state of a Draft DOI is final. Draft DOIs remain until the DOI owner either deletes them or converts them to another state. 
[block:api-header]
{
...
The [MDS API](doc:mds-api-guide) doesn't directly understand state, but you can change a DOI from Findable to Registered by using the `delete DOI` API call.
