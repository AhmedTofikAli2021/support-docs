...
The [DataCite GraphQL API](doc:datacite-graphql-api-guide) can be used for making direct queries into the graph, providing the ability to retrieve subsets of the graph and enabling more complex queries than are possible via the REST API, such as "two-hop" connections between objects.

The GraphQL API can be accessed via the API endpoint at <https://api.datacite.org/graphql>, and more documentation on usage is available in the [DataCite GraphQL API Guide](doc:datacite-graphql-api-guide).
