...
The *DOI created* date is the date the DOI is registered in the DataCite system. The *DOI registered* date refers to when it was "registered" in the global handle server. If using a "draft" state workflow, the created and registered date and time could be different.