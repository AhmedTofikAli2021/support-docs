---
title: 'Use Case: California Digital Library, DASH repository'
excerpt: ''
deprecated: false
hidden: false
metadata:
...
{
  "type": "warning",
  "body": "This document is the  How-To Guide as described by the California Digital Library implementation of Make Data Count. It describes their experience and implementation in detail. Tips and tools (e.g. a Python Log Processor) are detailed in this guide and available on our public Github.",
  "title": "California Digital Library implementation of Make Data Count"
}
[/block]
# Getting Started Implementing the COUNTER Code of Practice for Research Data in Repositories

The [COUNTER Code of Practice for Research Data (CoP)](https://docs.google.com/document/d/1T5YipZckpyu6ol4IqUw4vNK6uZCLsNpcFwD46R492BE/edit#heading=h.64asv91yv7sz) is a community developed recommendation for standardized data usage metrics. The below guide outlines how to implement this at the repository level, as done by the California Digital Library Dash platform. For a general overview on the project and recommendation:
  ...
  * Read through the CoP preprint [here](https://peerj.com/preprints/26505/)
...
For specifics about the logging format CDL is using and that works with the counter-processor, please see the readme and sample log in that Github repository.
...
CDL uses a service called [freegeoip.net](https://ipstack.com/) for IP to location lookups. It is free, has code available on GitHub, is community supported and allows a generous number of API calls per hour (10,000) to their already existing API server. If more queries are needed it could be hosted on a local server instead. This service provides country, state and often city/locality IP address geolocation. Right now we're only recording country-level information but may record more granular information in the future.
...
[block:parameters]
{
  "data": {
    ...
    "0-3": "Citation",
    "1-0": "API Base URL",
    "1-1": "(https://api.datacite.org/events)",
    "1-2": "(https://api.datacite.org/events)",
    "1-3": "(https://api.eventdata.crossref.org/v1/events)",
    "2-0": "Request parameters",
    "2-1": "mailto= source-id=datacite-usage obj-id=i",
    "2-2": "mailto= source-id=datacite-usage obj-id=",
    "2-3": "mailto= rows=10000\nsource=datacite subj-id=\n\nsource=crossref obj-id=",
    "3-0": "Processing",
    "3-1": "Sum for all months for ['attributes']['total'] where ['attributes']['relation-type-id'] is unique-dataset-investigations-regular or unique-dataset-investigations-machine",
    "3-2": "Sum for all months for ['attributes']['total'] where ['attributes']['relation-type-id'] is unique-dataset-requests-regular or unique-dataset-requests-machine",
    "3-3": "Combine, filter and deduplicate two queries and then get count of records. One query is for source=datacite and the other for source=crossref (more info below)"
  },
  "cols": 4,
  "rows": 4
}
...
#Other repositories experiences

The following webinar presents other repositories experiences on implementing the CoP.

[block:html]
{
  "html": "<iframe src=\"https://player.vimeo.com/video/326615258\" width=\"640\" height=\"328\" frameborder=\"0\" allow=\"autoplay; fullscreen\" allowfullscreen></iframe>\n<p><a href=\"https://vimeo.com/326615258\">Repository Implementation Webinar</a> from <a href=\"https://vimeo.com/user71165963\">Make Data Count</a> on <a href=\"https://vimeo.com\">Vimeo</a>.</p>"
}
[/block]