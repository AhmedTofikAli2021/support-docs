...
You can display usage in different ways. In [DataCite Commons](doc:datacite-commons) and the [Data Metrics Badge](docs:displaying-usage-and-citations-in-your-repository), DataCite has opted for displaying `unique-dataset-investigations-regular` as `views` and `unique-dataset-requests-regular` as `downloads`.
